...
<cfcomponent name="MeldMiniCommerceRemoteAOP" 
			displayname="MeldMiniCommerceRemoteAOP:RemoteProxyBean" 
			hint="Abstract Base Class for Aop Based Remote Proxy Beans" 
			output="false">
			
	<cfset variables.proxyId = CreateUUId() />
	<cfset variables.beanFactoryName = "MeldMiniCommerceBeanFactory" />
		...
		<cflock name="RemoteProxyBean.#variables.proxyId#.Setup" type="readonly" timeout="5">
			<cfif not StructKeyExists(variables, "constructed") or not variables.constructed>
				...
				<cftry>		
					...
					<cfset remoteFactory = bf.getBean("&MeldMiniCommerceRemoteAOP") />
					<cfset variables.target = bf.getBean("MeldMiniCommerceRemoteAOP") />
	...
	<cffunction name="getMerchantList" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getMerchantList', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>

<cffunction name="getTransactionList" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getTransactionList', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>

<cffunction name="getPaymentTypeList" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getPaymentTypeList', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>
